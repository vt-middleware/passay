...
public class MemoryMappedFileWordListTest extends AbstractWordListTest<MemoryMappedFileWordList>
{
  ...
  {
    try {
      new MemoryMappedFileWordList(new RandomAccessFile(Dictionaries.FREEBSD_FILE_SORTED, "r"), true, -1);
    ...
    try {
      new MemoryMappedFileWordList(new RandomAccessFile(Dictionaries.FREEBSD_FILE_SORTED, "r"), true, 100 + 1);
      fail("Should have thrown IllegalArgumentException");
    } catch (IllegalArgumentException e) {
      assertThat(e).isExactlyInstanceOf(IllegalArgumentException.class);
    } catch (Exception e) {
    ...
    MemoryMappedFileWordList fwl = new MemoryMappedFileWordList(
      new RandomAccessFile(Dictionaries.FREEBSD_FILE_SORTED, "r"), true, 0);
    fwl.close();

    fwl = new MemoryMappedFileWordList(new RandomAccessFile(Dictionaries.FREEBSD_FILE_LOWERCASE_SORTED, "r"), false, 0);
    fwl.close();
  }
}
