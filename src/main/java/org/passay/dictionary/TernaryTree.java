...
public class TernaryTree
{
  ...
  // CheckStyle:FinalParametersCheck ON
  {
    if (index < word.length()) {
      ...
      final String split = PasswordUtils.toString(node.getSplitChar());
      final int cmp = comparator.compare(PasswordUtils.toString(cp), split);
      if (cmp < 0) {
    ...
    final boolean fullPath, final StringBuilder buffer)
  {
    if (node != null) {
      ...
      if (node.getEqkid() != null) {
        ...
        printNode(node.getEqkid(), s + '-' + PasswordUtils.toString(cp) + suffix, depth + 1, fullPath, buffer);
      } else {
        ...
        buffer.append(line).append('-').append(PasswordUtils.toString(cp)).append(TernaryTree.LINE_SEPARATOR);
      }
