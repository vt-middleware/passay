...
/**
 * Provides fast searching for dictionary words using a ternary tree. The entire
 * dictionary is stored in memory, so heap size may need to be adjusted to
 * accommodate large dictionaries. It is highly recommended that sorted word
 * lists be inserted using their median. This helps to produce a balanced
 * ternary tree which improves search time. This class inherits the lower case
...
public class TernaryTreeDictionary implements Dictionary
{
  ...
  /**
   * Creates a new balanced tree dictionary from the supplied {@link WordList}.
  ...
  /**
   * Creates a dictionary that uses the supplied ternary tree for dictionary
   * searches.
   *
  ...
  /**
   * Returns an array of strings which partially match the supplied word. This
   * search is case sensitive by default. See {@link TernaryTree#partialSearch}.
   *
  ...
  /**
   * Returns an array of strings which are near to the supplied word by the
   * supplied distance. This search is case sensitive by default. See {@link
